...
import static ca.islandora.alpaca.triplestore.processors.FcrepoHeaders.FCREPO_URI;
...
import ca.islandora.alpaca.triplestore.processors.SparqlDeleteProcessor;
import ca.islandora.alpaca.triplestore.processors.SparqlUpdateProcessor;
import ca.islandora.alpaca.event.AS2Event;
import ca.islandora.alpaca.event.AS2Url;
import ca.islandora.alpaca.exceptions.MissingDescribesUrlException;
import ca.islandora.alpaca.exceptions.MissingJsonldUrlException;
import ca.islandora.alpaca.exceptions.MissingPropertyException;

/**
 * TriplestoreIndexer is a Camel route that listens for messages and indexes the objects into the configured
 * triplestore.
 * @author dhlamb
 */
public class TriplestoreIndexer extends RouteBuilder {
    ...
    private final TriplestoreIndexerOptions triplestoreConfig;

    private final AlpacaConfig alpacaConfig;

    public TriplestoreIndexer(
            final TriplestoreIndexerOptions triplestoreConfig,
            final AlpacaConfig alpacaConfig) {
        this.triplestoreConfig = triplestoreConfig;
     ...
     */
    private String getTriplestoreBaseUrl() {
        return alpacaConfig.addHttpOptions(triplestoreConfig.getBaseUrl());
    }
    ...
    public void configure() {
        ...
        onException(Exception.class)
            .maximumRedeliveries(alpacaConfig.getMaxRedeliveries())
            .log(
                ERROR,
                LOGGER,
        ...
        from(triplestoreConfig.getJmsIndexStream())
            .routeId("IslandoraTriplestoreIndexer")
              ...
              .setBody(simple("${null}"))
              .toD(alpacaConfig.addHttpOptions("${exchangeProperty.jsonld_url}", true))
              ...
              .to(getTriplestoreBaseUrl());

        from(triplestoreConfig.getJmsDeleteStream())
